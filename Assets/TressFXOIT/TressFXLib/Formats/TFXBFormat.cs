...
namespace TressFXLib.Formats
{
    ...
    public class TFXBFormat : IHairFormat
    {
        ...
        public virtual HairMesh[] Import(BinaryReader reader, string path, Hair hair, HairImportSettings importSettings)
        {
            ...
            /*int numFollowHairsPerOneGuideHair = */
            reader.ReadInt32();
            ...
            // data process 
            // 处理位移，旋转，比例，
            Vector3 translate = hair.boundingSphere.center - importSettings.position * 0.01f;
            hair.boundingSphere.center = new Vector3(0, 0, 0) + importSettings.position * 0.01f;
            for (int i = 0; i < numVertices; i++)
            {
                Vector3 vtPos = new Vector3(vertices[i].x, vertices[i].y, vertices[i].z);
                vtPos -= translate;
                vertices[i] = new Vector4(vtPos, vertices[i].w);
            }
