...
namespace TriLib
{
    ...
    {
        public static AssetDownloader Single;
        public static AssetDownloader Inst
        {
            get
            {
                if (Single == null)
                {
                    GameObject obj = new GameObject("AssetDownloader");
                    DontDestroyOnLoad(obj);
                    Single = obj.AddComponent<AssetDownloader>();
                }
                return Single;
            }
        }
       ...
       // public Action<bool> actt;
        ...
        protected void Start()
        {
          //  if (AutoStart && !string.IsNullOrEmpty(AssetURI) && !string.IsNullOrEmpty(AssetExtension))
            {
            //   DownloadAsset(AssetURI, AssetExtension, null, null, null, WrapperGameObject, ProgressCallback,(f)=> {
              //     if (f) { Debug.Log("2222222222"); }
            //   });
            }
        }
        ...
        public bool DownloadAsset(string assetURI, string assetExtension, ObjectLoadedHandle onAssetLoaded = null, TexturePreLoadHandle onTexturePreLoad = null, AssetLoaderOptions options = null, GameObject wrapperGameObject = null, AssimpInterop.ProgressCallback progressCallback = null,Action<bool> act = null)
        {
           ...
           // actt = act;
            StartCoroutine(DoDownloadAsset(assetURI, assetExtension, onAssetLoaded, options, wrapperGameObject, progressCallback,act));
            return true;
        }
        ...
        private IEnumerator DoDownloadAsset(string assetUri, string assetExtension, ObjectLoadedHandle onAssetLoaded,
            ...
            AssimpInterop.ProgressCallback progressCallback = null,Action<bool> act = null)
        {
                ...
                try
                {
                    if (Async)
                    {
                        using (var assetLoaderAsync = new AssetLoaderAsync())
                        {
                            try
                            {
                                assetLoaderAsync.LoadFromMemoryWithTextures(data, assetExtension, options, wrapperGameObject, onAssetLoaded, null, null, null, progressCallback,act);
                                Debug.Log("执行完毕1");
                                //act?.Invoke(true);
                            }
                            catch (Exception e)
